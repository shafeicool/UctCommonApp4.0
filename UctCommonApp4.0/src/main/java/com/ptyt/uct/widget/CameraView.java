...
import com.ptyt.uct.activity.MessageShootActivity;
...
public class CameraView extends View {
    ...
    private void init(Context context) {
        ...
        MessageShootActivity activity = (MessageShootActivity) context;
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (event.getPointerCount() == 1 && action == MotionEvent.ACTION_DOWN) {
            float x = event.getX();
            float y = event.getY();
            /* ptyt begin, 缩小聚焦的范围，防止屏幕上方和下方有其他控件的地方也能点击_shafei_20170921 */
            if (y < (screenHeight / 8) || y > (screenHeight / 3 * 2)) {
                return true;
            }
            /* ptyt end */
            setFoucsPoint(x, y);
