...
public abstract class BasePagingAdapter<T> extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public int getItemCount() {
        if(!isAddHeadView){
            return lists.size() < 10 ? lists.size() : (lists.size() + 1);//当没数据时隐藏footView
        }else {
            return lists.size() < 10 ? lists.size()+1 : (lists.size() + 2);//当没数据时隐藏footView
        }
    }
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if (viewType == ConstantUtils.FOOTVIEW_TYPE) {
            View view = View.inflate(context, R.layout.item_footview, null);
